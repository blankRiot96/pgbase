...
class RenderedFont:
    ...
    def set_text(self, text):
        ...
        old_text = self.text
        self.text = text
        # only render if the text changes
        self.need_to_render = ((self.text != old_text) or self.need_to_render)

    def set_color(self, color):
        if isinstance(color, (str, int)):
           pass

        elif isinstance(color, t.Sequence):
            if len(color) < 3 or len(color) > 4:
                raise ValueError("Color Sequence must be either 3 or 4 elements long")

            for elem in color:
                if not isinstance(elem, int):
                    raise TypeError("Color sequence must be a sequence of integers")
                elif elem < 0 or elem > 255:
                    raise ValueError("Color component must be in range [0, 255]")
        else:
            raise TypeError("Color must be pygame.Color-compatible")

        old_color = self.color

        self.color = color
        # only render if the color changes
        self.need_to_render = ((self.color != old_color) or self.need_to_render)

    def set_antialias(self, antialias):
        old_antialias = self.antialias
        self.antialias = True if antialias else False
        # only render if the AA changes
        self.need_to_render = ((self.antialias != old_antialias) or self.need_to_render)

    def render_text(self):
        if self.color is None:
            raise RuntimeError("Need to set color before rendering text")
